...
static void send_specify_cmnd(void) {
    ...
    for (unsigned i = 0; i < sizeof(bytes_to_send)/sizeof(bytes_to_send[0]); i++) {
        if (((msr >> m_MSR_DIO)&1) == 1) {
            k_printf("Problem sending specify command to floppy disk. i = %u\n", i);
            return;
        }
...
static void configure_floppy(void) {
    ...
    //Implied seek on, FIFO on, driving polling mode off, threshold = 8, precomp = 0
...
void FLOPPY_144_reset(void) {
    ...
    dor |= 1 << m_DOR_IRQ;
    IO_out_port_b(FR_DIGITAL_OUTPUT_REGISTER, dor);*/
    volatile uint8_t dor = (0 << m_DOR_D_SEL) | (1 << m_DOR_IRQ) | (1 << m_DOR_RESET);
    IO_out_port_b(FR_DIGITAL_OUTPUT_REGISTER, dor);
...
static void read_sector(struct CHS location, unsigned drive, void *data_out) {
    ...
    uint8_t bytes_to_send[] = {
        ...
        location.sector,
        2,
        (location.sector+1) >= m_FLOPPY_144_SECTORS_PER_TRACK ? m_FLOPPY_144_SECTORS_PER_TRACK : location.sector+1,
        0x1b,
        0xff,
        };
    ...
    for (unsigned i = 0; i < sizeof(bytes_to_send)/sizeof(bytes_to_send[0]); i++) {
        if (((msr >> m_MSR_DIO)&1) == 1) {
            k_printf("Problem with reading a sector from floppy disk. i = %u\n", i);
            return;
        }
        IO_out_port_b(FR_DATA_FIFO, bytes_to_send[i]);
        do {
    ...
    msr = IO_in_port_b(FR_MAIN_STATUS_REGISTER);
    while (((msr >> m_MSR_RQM)&1) != 1) {
        if (((msr >> m_MSR_DIO)&1) != 1 || ((msr >> m_MSR_CB)&1) != 1) {
            k_printf("Problem with reading a sector from floppy disk. 3\n");
            return;
        }
    ...
    if (((msr >> m_MSR_RQM)&1) != 1 || ((msr >> m_MSR_DIO)&1) != 0 || ((msr >> m_MSR_CB)&1) != 0) {
...
__asm__ (
   ".globl FLOPPY_144_irq_handler\n"
   "FLOPPY_144_irq_handler:\n"
   "   pusha               \n" /* Save all registers               */
