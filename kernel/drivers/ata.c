...
struct ATA_Device ATA_device = {
    0x1f0, 0x3f6, 0x4, 0x5, 0x6
};

void ATA_soft_reset(struct ATA_Device *ctrl) {

    volatile uint8_t dev_ctrl_reg = IO_in_port_b(ctrl->dev_ctl+CTRL_DEVICE_CTRL_REG);
...
void ATA_read_sector(struct ATA_Device *ctrl, void *data) {
    ...
    volatile uint8_t drive_head_reg = IO_in_port_b(ctrl->base+IO_DRIVE_HEAD_REG);
