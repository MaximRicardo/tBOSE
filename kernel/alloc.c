...
void* alloc_free_unused_area_in_mem_map_entry(uint32_t size, const struct MEMORY_MAP_Entry *mem_map_entry_ptr) {
    ...
    while (cur_entry_ptr->next_entry_ptr != NULL) {
        struct AllocEntry *next_entry_ptr = cur_entry_ptr->next_entry_ptr;

        uint32_t entries_gap_size = next_entry_ptr->region.base - (uint32_t)get_region_end(&cur_entry_ptr->region) - 1;
...
void* alloc_free_unused_area(uint32_t size) {
    ...
    for (unsigned i = 0; i < MEMORY_MAP_descriptor_ptr->n_entries; i++) {
        const struct MEMORY_MAP_Entry *cur_mem_map_entry_ptr = &MEMORY_MAP_entries[i];
        if (cur_mem_map_entry_ptr->type != m_MEMORY_MAP_ENTRY_FREE_TYPE) continue;

        void *free_area = alloc_free_unused_area_in_mem_map_entry(size, cur_mem_map_entry_ptr);
        if (free_area) return free_area;
    }
