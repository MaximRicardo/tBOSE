...
__attribute__((noreturn)) void k_main_setup_done();

__attribute__((noreturn)) void
k_main(const struct VBE_Info *old_vbe_info_ptr,
       const struct VBE_ModeInfo *old_vbe_mode_info_ptr, uint32_t boot_disk_arg)
{
	...
	idt.descriptor.size =
		2048; //The IDT table is 2048 bytes large (8 bytes per entry * 256 entries)
	idt.descriptor.base =
		(uint32_t)(&idt.entries[0]); //Starts at the first entry
	...
	page_directory[m_PAGE_TABLES_TABLE_VIRTUAL_ADDRESS / m_PAGE_TABLE_SIZE] =
		(uint32_t)page_tables_table | 0x3;
	...
	for (unsigned i = 2; i < 1023; i++) {
		...
		uint32_t page_dir_flags = i * 1048576 * 4 < 0xc0000000 ? 0x7 :
									 0x7;
		PAGE_create_table((void *)(4096 * 1024 * i),
				  (void *)(4096 * 1024 * i), page_table_virt,
	...
	{
		uint32_t *page_table_phys =
			(uint32_t *)(page_tables_table[0] & -4096);
		uint32_t *page_table_virt =
			(uint32_t *)m_PAGE_TABLES_TABLE_VIRTUAL_ADDRESS;
	...
	//mapped correctly.
	{
		...
		PAGE_create_table((void *)0, (void *)0xc0000000,
				  page_table_virt, page_table_phys,
				  page_directory, 0x7, 0x7);
	}
	...
	//Mapping the framebuffer to 0xf0000000
	{
		...
		PAGE_create_table((void *)(VBE_mode_info.framebuffer /
					   m_PAGE_TABLE_SIZE *
					   m_PAGE_TABLE_SIZE),
...
void k_main_setup_done()
{
	...
	for (unsigned i = 0; i < MEMORY_MAP_descriptor->n_entries; i++) {
		k_printf("ENTRY #%u\n", i);
		k_printf("base lo = 0x%lx ",
			 (unsigned long)MEMORY_MAP_entries[i].base_lo);
		k_printf("base hi = 0x%lx ",
			 (unsigned long)MEMORY_MAP_entries[i].base_hi);
		k_printf("size lo = 0x%lx ",
			 (unsigned long)MEMORY_MAP_entries[i].length_lo);
		k_printf("size hi = 0x%lx ",
			 (unsigned long)MEMORY_MAP_entries[i].length_hi);
		k_printf("type = %ld ",
			 (unsigned long)MEMORY_MAP_entries[i].type);
		k_printf("unused = 0x%lx ",
			 (unsigned long)MEMORY_MAP_entries[i].unused_field);
